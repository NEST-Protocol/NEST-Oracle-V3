...
contract Nest_NToken_OfferMain {
    ...
    Nest_NToken_OfferPrice _offerPrice;                                 //  价格合约
    ...
    //  log,报价合约, token地址,eth数量,erc20数量,延时区块,是否挖矿
    event OfferContractAddress(address contractAddress, address tokenAddress, uint256 ethAmount, uint256 erc20Amount, uint256 continued,bool isMining);         
    ...
    constructor (address voteFactory) public {
        ...
        _offerPrice = Nest_NToken_OfferPrice(address(_voteFactory.checkAddress("nest.nToken.offerPrice")));            
    ...
    function createOffer(uint256 ethAmount, uint256 erc20Amount, address erc20Address, uint32 delayedBlock, bool isDeviate, bool isMining) private {
        ...
        // 创建报价合约
        emit OfferContractAddress(toAddress(_prices.length), address(erc20Address), ethAmount, erc20Amount,delayedBlock,isMining);
        ...
        // 记录价格
        _offerPrice.addPrice(ethAmount, erc20Amount, block.number.add(delayedBlock), erc20Address, address(msg.sender));
    }
    ...
    */
    function sendEthBuyErc(uint256 ethAmount, uint256 tokenAmount, address contractAddress, uint256 tranEthAmount, uint256 tranTokenAmount, address tranTokenAddress) public payable {
        ...
        // 修改价格
        _offerPrice.changePrice(tranEthAmount, tranTokenAmount, tranTokenAddress, offerPriceData.blockNum.add(offerPriceData.delayedBlock));
    ...
    */
    ...
    function comparativePrice(uint256 myEthValue, uint256 myTokenValue, address token) private returns(bool) {
        ...
        uint256 myPrice = myTokenValue.mul(1 ether).mul(1 ether).div(myEthValue);
        uint256 tranTopPrice = frontTokenValue.mul(1 ether).mul(1 ether).mul(uint256(100).add(_deviate)).div(frontEthValue.mul(100));
        uint256 tranBottomPrice = frontTokenValue.mul(1 ether).mul(1 ether).mul(uint256(100).sub(_deviate)).div(frontEthValue.mul(100));
    ...
    // 查看区块报价次数
    function checkBlockOfferTimes(uint256 blockNum, address token) public view returns (uint256) {
        return _blockOfferTimes[blockNum][token];
    }
    ...
    // 查看报价挖矿数量
    function checkOfferMining(uint256 blockNum, address token) public view returns (uint256) {
        return _blockMining[blockNum][token].div(_blockOfferTimes[blockNum][token]);
    }
...
//  价格合约
interface Nest_NToken_OfferPrice {
    //  增加价格数据
