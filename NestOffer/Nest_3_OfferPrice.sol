...
contract Nest_3_OfferPrice{
    ...
    struct TokenInfo {                                              //  token报价信息
        ...
        uint256 priceCostLeast;                                     //  价格eth最少费用
        uint256 priceCostMost;                                      //  价格eth最多费用 
        uint256 priceCostSingle;                                    //  价格eth单条数据费用
        uint256 priceCostUser;                                      //  价格eth费用用户比例
    }
    uint256 destructionAmount = 10000 ether;                        //  调用价格销毁 nest 数量 1万
    uint256 effectTime = 10 minutes;                                //  可以调用价格等待时间 1 days
    ...
    */
    function updateAndCheckPriceNow(address tokenAddress) public payable returns(uint256 ethAmount, uint256 erc20Amount, uint256 blockNum) {
        ...
        while((checkBlock >= block.number || tokenInfo.priceInfoList[checkBlock].ethAmount == 0) && checkBlock > 0) {
...
interface NToken {
    ...
    function transferFromForOfferPrice(address from, address to, uint256 value) external returns (bool);
}
