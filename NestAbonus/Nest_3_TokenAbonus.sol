...
contract Nest_3_TokenAbonus {
    ...
    */
    function depositIn(uint256 amount, address token) public {
        ...
        if (nowTime < nextTime) {
            ...
            uint256 startTime = _nextTime.add((times).mul(_timeLimit));                                                              //  计算应该分红的时间
            uint256 endTime = startTime.add(_getAbonusTimeLimit);                                                                    //  计算应该停止分红的时间
            require(!(nowTime >= startTime && nowTime <= endTime));
        }
        _tokenSave.depositIn(amount, token, address(msg.sender));                                                                    //  存入
    }
    ...
    */
    function takeOut(uint256 amount, address token) public {
        ...
        _tokenSave.takeOut(amount, token, address(msg.sender));                                                                       //  转出
    }                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           
    ...
    */
    function getAbonus(address token) public {
        ...
        reloadTime();
        reloadToken(token);                                                                                                           //  时间超过优先结算，结算后分红
        uint256 nowTime = now;
        require(nowTime >= _nextTime.sub(_timeLimit) && nowTime <= _nextTime.sub(_timeLimit).add(_getAbonusTimeLimit), "Not time to draw");
        require(!_getMapping[_times.sub(1)][token][address(msg.sender)], "Have received");                                            //  上期分红没有领取记录 
        _tokenSelfHistory[token][_times.sub(1)][address(msg.sender)] = tokenAmount;                                                   //  个人锁仓token快照
    ...
    function levelingResult(address token) private {
        ...
        if (thisAbonus > minimumAbonus) {
            ...
            if (thisAbonus > 5000 ether) {
                levelAmount = thisAbonus.mul(_savingLevelThree).div(100).sub(600 ether);
            } else if (thisAbonus > 1000 ether) {
                levelAmount = thisAbonus.mul(_savingLevelTwo).div(100).sub(100 ether);
            } else {
    ...
    * @param num 阈值
    */
    function checkExpectedSpanForNest(uint256 num) public onlyOwner {
    ...
    * @param num 阈值
    */
    function checkExpectedSpanForNToken(uint256 num) public onlyOwner {
